...
Функция ВыполнитьИнкрементальнуюЗагрузкуКонфигурации(ПараметрыИнкрементальнойЗагрузки) Экспорт
	...
	СтрокаПодключения = ПолучитьСтрокуПодключения(АдресСервера, ИмяБазы, Пользователь, Пароль);
...
Функция ПолучитьСтрокуПодключения(АдресСервера, ИмяБазы, Пользователь, Пароль)
	...
	ШаблонСтроки = "/IBConnectionString""Srvr='{{АдресСервера}}'; Ref='{{ИмяБазы}}'""", "{{Пользователь}}", "{{Пароль}}";
	...
	СтрокаПодключения = СтрЗаменить(СтрокаПодключения, "{{Пользователь}}", Пользователь);
	СтрокаПодключения = СтрЗаменить(СтрокаПодключения, "{{Пароль}}", Пароль);
